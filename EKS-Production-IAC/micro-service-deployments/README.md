...
> **Testing on Minikube first?** See `k8s/minikube/README.md` for a
> dedicated, step-by-step guide (in-cluster Postgres, locally-built images,
> no AWS account needed) - the steps below are for the real EKS/RDS/ECR path.
...
2. **Provision RDS PostgreSQL** (one instance shared across the 4 DBs, matching compose). RDS creates the *instance*, not the databases inside it - connect once with `psql` and create the four your services expect before deploying anything:
   ```bash
   ...
   ```
   (Django's `migrate`, which each container runs on startup, only creates *tables* inside a database that already exists - it won't create the database itself the way our docker-compose init script did.)
3. **Create an S3 bucket** for product images (`AWS_STORAGE_BUCKET_NAME` in `01-secrets-configmap.yaml`), and the IAM role/policy for `product-service` to access it - see the setup comment at the top of `k8s/03-product-service.yaml` for the exact `eksctl` command and least-privilege policy scope.
4. **Apply manifests** (`k8s/` folder), in order:
   ```bash
   ...
   ```
5. Point `Route53` at the ALB the Ingress creates (requires the AWS Load
   Balancer Controller add-on installed on the cluster).
6. Update the Amplify env var `VITE_API_GATEWAY_URL` to that domain and redeploy the frontend.

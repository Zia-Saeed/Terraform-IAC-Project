...
# S3 + Rekognition + SQS Permissions
resource "aws_iam_policy" "lambda_processing_policy" {
  name        = "lambda-processing-policy-for-image-processing-project"
  description = "Policy for Lambda to process SQS messages with S3 and Rekognition"
    ...
    Statement = [
      {
        ...
        Resource = aws_sqs_queue.sqs_main_fifo.arn
      },
      {
      ...
      {
        Sid    = "AllowWriteDestBucketCropped"
        Effect = "Allow"
        Action = [
            "s3:PutObject",
            "s3:PutObjectAcl"
